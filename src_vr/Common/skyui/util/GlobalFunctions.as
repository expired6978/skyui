...
class skyui.util.GlobalFunctions
{
  /* PUBLIC FUNCTIONS */
	...
	public static function getMappedKey(a_control: String, a_context: Number, a_bGamepad: Boolean): Number
	{
		...
		if(skse.IsUsingMotionControllers()) {
			var isPrimary = !skse.IsLeftHanded();
			switch(skse.GetMotionControllerType()) {
				case Input.MOTION_CONTROLLER_VIVE:
				return skse.GetMappedKey(a_control, isPrimary ? Input.DEVICE_VIVE_PRIMARY : Input.DEVICE_VIVE_SECONDARY, a_context);
				break;
				case Input.MOTION_CONTROLLER_OCULUS:
				return skse.GetMappedKey(a_control, isPrimary ? Input.DEVICE_OCULUS_PRIMARY : Input.DEVICE_OCULUS_SECONDARY, a_context);
				break;
				case Input.MOTION_CONTROLLER_WINDOWS_MR:
				return skse.GetMappedKey(a_control, isPrimary ? Input.DEVICE_WINDOWS_MR_PRIMARY : Input.DEVICE_WINDOWS_MR_SECONDARY, a_context);
				break;
			}
		} else if (a_bGamepad == true) {
