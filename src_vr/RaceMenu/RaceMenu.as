...
class RaceMenu extends MovieClip
{
	...
	private var ITEMLIST_HEIGHT_FULL = 825; // 528
	private var ITEMLIST_HEIGHT_SHARED = 639; // 335
	...
	private function onLoad()
	{
		...
		_global["skse"] = {
			plugins:{
				CharGen:{
					bEnableSculpting:true,
					GetBrushes: function() {
						return [
							{type: 0, radius: 0.1, radiusMin: 0, radiusMax: 1.0, radiusInterval: 0.01, strength: 0.1, strengthMin: 0, strengthMax: 1.0, falloff: 0.1, falloffMin: 0, falloffMax: 1, falloffInterval: 0.01, mirror: false},
							{type: 1, radius: 0.1, radiusMin: 0, radiusMax: 1.0, radiusInterval: 0.01, strength: 0.1, strengthMin: 0, strengthMax: 1.0, falloff: 0.1, falloffMin: 0, falloffMax: 1, falloffInterval: 0.01, mirror: false},
							{type: 2, radius: 0.1, radiusMin: 0, radiusMax: 1.0, radiusInterval: 0.01, strength: 0.1, strengthMin: 0, strengthMax: 1.0, falloff: 0.1, falloffMin: 0, falloffMax: 1, falloffInterval: 0.01, mirror: false},
							{type: 3, radius: 0.1, radiusMin: 0, radiusMax: 1.0, radiusInterval: 0.01, strength: 0.1, strengthMin: 0, strengthMax: 1.0, falloff: 0.1, falloffMin: 0, falloffMax: 1, falloffInterval: 0.01, mirror: false},
							{type: 4, radius: 0.1, radiusMin: 0, radiusMax: 1.0, radiusInterval: 0.01, strength: 0.1, strengthMin: 0, strengthMax: 1.0, falloff: 0.1, falloffMin: 0, falloffMax: 1, falloffInterval: 0.01, mirror: false},
							{type: 5, radius: 0.1, radiusMin: 0, radiusMax: 1.0, radiusInterval: 0.01, strength: 0.1, strengthMin: 0, strengthMax: 1.0, falloff: 0.1, falloffMin: 0, falloffMax: 1, falloffInterval: 0.01, mirror: false}
						];
					},
					...
					GetMeshes: function() {
						return [
							{name: "Head", meshIndex: 0, wireframe: true, locked: false, visible: true, morphable: true, vertices: 512, wfColor: 0xFFFF00FF},
							{name: "Mouth", meshIndex: 0, wireframe: true, locked: false, visible: true, morphable: true, vertices: 512, wfColor: 0xFFFF00FF},
							{name: "Hair", meshIndex: 0, wireframe: true, locked: false, visible: true, morphable: true, vertices: 512, wfColor: 0xFFFF00FF},
							{name: "Brow", meshIndex: 0, wireframe: true, locked: false, visible: true, morphable: true, vertices: 512, wfColor: 0xFFFF00FF},
							{name: "Eyes", meshIndex: 0, wireframe: true, locked: false, visible: true, morphable: true, vertices: 512, wfColor: 0xFFFF00FF}
						];
					},
					GetExternalFiles: function() {
						return [
							{name: "TestFile", path: "Data/SKSE/TestFile.jslot", size: 10, lastModified: new Date(), directory: false}
						];
					},
					ReadPreset: function(a_path: String, a_target: Object, json: Boolean): Boolean
					{
						a_target["mods"] = [{name: "Something.esp", loadedIndex: 0}];
						a_target["headParts"] = ["MyPart1", "MyPart2"];
						a_target["weight"] = {name: "Weight", value: 1.0};
						a_target["hair"] = {name: "Pogo", value: 1.0};
						a_target["tints"] = [{color: 0xFF00FF00, index: 0, texture: "Test.dds"}];
						
						a_target["morphs"] = [];
						for(var i = 0; i < 40; i++)
						{
							a_target["morphs"].push({name: "TestMorph" + 1, value: 1.0, index: i, type: 0});
						}
						return false;
					}
				}
	...
	public function SetPlatform(a_platform: Number, a_bPS3Switch: Boolean): Void
	{
		...
		_acceptControl = {keyCode: GlobalFunctions.getMappedKey("Cancel", Input.CONTEXT_RACESEXMENU, a_platform != 0)};
		...
		if(_platform == 0) {
			...
			_textureControl = {keyCode: GlobalFunctions.getMappedKey("Delete Save", Input.CONTEXT_DIALOGUEMENU, a_platform != 0)};
		} else {
	...
	{
		skyui.util.Debug.dump("handleInput", details);
		
		if(_global.skse.IsVR()) {
			if(_pauseInputHandling)
				return false;

			skyui.util.Input.rateLimit(this, "_pauseInputHandling", 30);
		}
	...
	private function updateItemDescriptor(a_fadeDelay: Number): Void
	{
		...
		if(selectedEntry.extraData && itemList.disableInput == false) {
			...
			if(formName != undefined && formId != undefined) {
				...
				itemDescriptor._y = bottomBar._y - itemDescriptor._height;
